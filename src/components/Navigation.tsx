      ...
      >
        <div className="flex items-center justify-between px-4 md:px-6 lg:px-margin-desktop max-w-container-max mx-auto w-full gap-4">
          ...
          <div className="flex-shrink-0">
            <Link to="/" className="font-display-lg text-headline-lg-mobile md:text-headline-lg text-primary tracking-tight hover:text-secondary transition-colors duration-300 z-10 whitespace-nowrap">
          ...
          {/* Right Side */}
          <div className="flex-shrink-0 flex justify-end items-center gap-3 md:gap-4 z-10">
            ...
            <button
              className="lg:hidden flex flex-col items-center justify-center w-10 h-10 rounded-xl bg-surface-container/50 backdrop-blur-md border border-outline-variant/20 hover:bg-surface-container transition-colors relative z-50"
              onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
              aria-label="Toggle navigation menu"
            >
              <span className={`block w-5 h-0.5 bg-primary rounded-full transition-all duration-300 ${mobileMenuOpen ? 'rotate-45 translate-y-[3px]' : ''}`} />
              <span className={`block w-5 h-0.5 bg-primary rounded-full transition-all duration-300 mt-1 ${mobileMenuOpen ? 'opacity-0' : ''}`} />
              <span className={`block w-5 h-0.5 bg-primary rounded-full transition-all duration-300 mt-1 ${mobileMenuOpen ? '-rotate-45 -translate-y-[5px]' : ''}`} />
            </button>
          </div>
        </div>
      </nav>
        ...
        >
          <div className="flex flex-col h-full pt-24 pb-8 px-6">
            ...
            <div className="space-y-3 pt-6 border-t border-outline-variant/20">
              <Link
                to="/cook-with-ai"
                className="flex items-center justify-center gap-2 bg-primary-container text-on-tertiary px-6 py-4 rounded-2xl font-label-sm text-base shadow-md shimmer-btn w-full"
              >
                <span className="material-symbols-outlined text-sm">bolt</span>
                Cook with AI
              </Link>
              <Link
                to="/profile"
                className="flex items-center justify-center gap-2 text-on-surface-variant hover:text-primary px-6 py-3 rounded-2xl border border-outline-variant/30 w-full transition-colors"
              >
